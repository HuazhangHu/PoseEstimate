...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        pe[:, 0::2] = torch.sin(position * div_term)  # transformer position embedding 使用正弦函數，位置維度對應曲綫
...
class MyModel(nn.Module):
    def __init__(self, num_frames):
        ...
        #                         padding=(3, 1, 1),
        #                         dilation=(3, 1, 1))
        # self.bn1 = nn.BatchNorm3d(136)
        # self.pool = nn.MaxPool3d(kernel_size=(1, 7, 7))

        self.sims = Sims()
        self.mha_sim = nn.MultiheadAttention(embed_dim=136, num_heads=4)  # 此处加了一层multi-head self-attention
    ...
    def forward(self, x, ret_sims=False):
        ...
        x = torch.cat([x1, x2], dim=1)  # similarity and self-attention concat
        ...
        return y1, y2  # y1 period,y2 periodicity
